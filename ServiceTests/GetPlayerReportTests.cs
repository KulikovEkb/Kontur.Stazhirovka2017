...
namespace Kontur.GameStats.Server.ServiceTests
{
    ...
    class GetPlayerReportTests
    {
        ...
        [Test]
        public void TestGet1PlayerReport()
        {
            using (var database = new LiteDatabase(AppDomain.CurrentDomain.BaseDirectory + "TestStorage.db"))
            {
                ...
                var result = Classes.PlayerReport.GetBestPlayers(matchesCollection, 1);
                Assert.AreEqual(1, result.Count);
                Assert.AreEqual("Player 1", result[0].Name);
                Assert.AreEqual(7, result[0].KillToDeathRatio);
            }
        }

        [Test]
        public void TestGet5PlayersReport()
        {
            using (var database = new LiteDatabase(AppDomain.CurrentDomain.BaseDirectory + "TestStorage.db"))
            {
                var matchesCollection = database.GetCollection<Classes.Match>("matches");
                ...
                var result = Classes.PlayerReport.GetBestPlayers(matchesCollection, 5);
                Assert.AreEqual(2, result.Count);
                Assert.AreEqual("Player 1", result[0].Name);
                Assert.AreEqual("Player 2", result[1].Name);
                Assert.AreEqual(7, result[0].KillToDeathRatio);
                Assert.AreEqual(2.216216, result[1].KillToDeathRatio);
            }
        }
    }
}
